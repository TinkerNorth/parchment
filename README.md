...
`paddingBottom` scrolling horizontally); `android:clipToPadding` moves the
cells over the padding but never the divider.
...
| `parchment_dividerSize` | dimension | Divider thickness along the scroll axis; without it the drawable's intrinsic size is used |
